...
# CONFIG
# -----------------------------
st.set_page_config(page_title="ProfitForge Pro v7", layout="wide")
st.title("🔥 ProfitForge Pro v7 - Multi-TF Live Signals + Backtest + Alerts")
...
# -----------------------------
def get_session():
    utc_now = datetime.now(timezone.utc) + timedelta(hours=1)  # UTC+1
    hour = utc_now.hour
...
# -----------------------------
# MULTI-TF SIGNAL SAFE
# -----------------------------
def multi_tf_signal(exchange_id, symbol):
    tf_scores = []
    for tf in ["5m","15m","1h","4h"]:
        df_tf = fetch_data(exchange_id, symbol, tf)
        if not df_tf.empty:
            _,score,_ = generate_signal(df_tf)
            tf_scores.append(score)
    return np.mean(tf_scores) if tf_scores else 50
...
if df.empty:
    ...
    score = multi_tf_signal(exchange_id, symbol)
