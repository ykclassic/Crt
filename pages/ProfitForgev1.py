...
import numpy as np
import os
import requests  # Added for reliable Telegram sending
from datetime import datetime, timedelta, timezone
import plotly.graph_objects as go
import itertools

# === YOUR DUMMY CREDENTIALS (WORKING) ===
TELEGRAM_TOKEN = "8367963721:AAH6B819_DevFNpZracbJ5EmHrDR3DKZeR4"
TELEGRAM_CHAT_ID = "865482105"  # Your personal chat ID

# Allow override via environment variables (good for future)
TELEGRAM_TOKEN = os.getenv("TELEGRAM_BOT_TOKEN", TELEGRAM_TOKEN)
TELEGRAM_CHAT_ID = os.getenv("TELEGRAM_CHAT_ID", TELEGRAM_CHAT_ID)

# -----------------------------
# RELIABLE TELEGRAM MESSAGE SENDER (SYNC)
# -----------------------------
def send_telegram_message(message: str):
    ...
    try:
        ...
        if response.status_code == 200:
            st.success("📩 Signal sent to Telegram!")  # Optional: shows in Streamlit UI
        else:
...
# -----------------------------
# REST OF YOUR ORIGINAL CODE (unchanged except signal sending)
# -----------------------------
...
for symbol in symbols:
    for tf in timeframes:
        ...
        # SEND TO TELEGRAM ONLY ON STRONG SIGNALS
        ...
        if run_backtest:
            ...
            for i,res in enumerate(top_results):
                ...
                st.markdown(f"Trades: {res['total_trades']} | Win Rate: {res['win_rate']:.1f}% | Total P/L: {res['total_pl']:.2f}")
