...
def generate_dashboard():
    ...
    # Logic is dynamic; results are not hardcoded.
    total_signals = len(df)
    
    # Check if 'status' column exists to avoid KeyError
    ...
    win_rate = round((wins / denominator * 100)) if denominator > 0 else 0
    ...
    # Dynamic styling for signal column
    if 'signal' in display_df.columns:
        display_df['signal'] = display_df['signal'].apply(
            lambda x: f'<b style="color:{"#3fb950" if x=="LONG" else "#f85149"}">{x}</b>'
        )
    
    # Generate HTML Table
    # Ensure only existing columns are passed to prevent KeyError
    cols_to_show = [c for c in ['ts', 'asset', 'signal', 'entry', 'tp', 'sl', 'Outcome'] if c in display_df.columns]
    table_html = display_df[cols_to_show].to_html(escape=False, index=False, border=0)
    ...
    """.replace("{{total_signals}}", str(total_signals)).replace("{{win_rate}}", str(win_rate))
